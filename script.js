...
=======
let dataset=[];
>>>>>>> ba3b11925729a6424f52b0bd6c348187530a44be
let chart;
let continuousChart;
...
fetch("api/kasus_mingguan.php?t=" + new Date().getTime())
...
function loadFilter() {
    ...
    const tahun = [...new Set(dataset.map(function(d) { return d.TAHUN; }))].sort();

    const newestYear = tahun[tahun.length - 1];
    ...
    // Set default to newest year from database
    t1.value = newestYear;
    t2.value = newestYear;
}
...
function updateSummaryCards() {
    // Get all unique years from dataset
    const allYears = [...new Set(dataset.map(function(d) { return d.TAHUN; }))].sort();
    const newestYearInDb = allYears[allYears.length - 1];
    const oldestYear = allYears[0];
    
    // Get current calendar year
    const currentYear = new Date().getFullYear().toString();
    
    // Check if current year exists in database, otherwise use newest year
    const tahunIni = allYears.includes(currentYear) ? currentYear : newestYearInDb;
    
    // Update Periode Data to show actual data range
    const periodeText = oldestYear + " - " + newestYearInDb;
    document.querySelector('.card-green .card-value').textContent = periodeText;

    // Total penyakit - count unique diseases
    ...
    // Calculate disease totals for TAHUN INI (current year or newest available year)
    const diseaseTotals = {};
    dataset.forEach(function(row) {
        // Only process data for tahun ini
        if (row.TAHUN !== tahunIni) return;
    ...
    document.getElementById("topDiseaseValue").textContent = "Total: " + topValue.toLocaleString() + " kasus (" + tahunIni + ")";
...
function updateChart() {
    ...
    if (tahun1 !== tahun2) {
        ...
        const sortedYears = [tahun1, tahun2].sort((a, b) => parseInt(a) - parseInt(b));
        yearText = sortedYears[0] + " & " + sortedYears[1];
    }
